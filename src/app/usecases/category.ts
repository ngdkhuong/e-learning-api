...
export const getCategoryByIdU = async (categoryId: string, categoryDbRepository: ReturnType<CategoryDbInterface>) => {
    if (!categoryId) {
        throw new AppError('Please provide valid category id', HttpStatusCodes.BAD_REQUEST);
    }
    const category = await categoryDbRepository.getCategoryById(categoryId);
    return category;
};

export const getAllCategoryU = async (categoryDbRepository: ReturnType<CategoryDbInterface>) => {
...
export const editCategoryU = async (
    ...
    if (!categoryInfo) {
        throw new AppError('Please provide valid data for editing a category', HttpStatusCodes.BAD_REQUEST);
    }
