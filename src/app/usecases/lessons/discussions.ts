...
export const getDiscussionsByLessonU = async (
    ...
    const discussions = await discussionDbRepository.getDiscussionByLesson(lessonId);
...
export const editDiscussionU = async (
    ...
    if (!discussionId) {
        throw new AppError('Please provide a discussion id', HttpStatusCodes.BAD_REQUEST);
    }
    message = message.trim();
    if (!message) {
        throw new AppError('Please provide a valid message', HttpStatusCodes.BAD_REQUEST);
    }
...
export const getRepliesByDiscussionIdU = async (
    ...
    const replies = await discussionDbRepository.getRepliesByDiscussionId(discussionId);
    return replies;
};
