...
export const rejectInstructorRequest = async (
    ...
    if (rejected) {
        throw new AppError('Already rejected this request', HttpStatusCodes.CONFLICT);
    }
...
export const blockInstructors = async (
    ...
    const response = await instructorRepository.blockInstructors(instructorId, reason);
