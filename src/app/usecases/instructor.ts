...
import { SavedInstructorInterface } from './../../types/instructorInterface';
...
export const changePasswordU = async (
    ...
    const instructor: SavedInstructorInterface | null = await instructorDbRepository.getInstructorById(id);

    if (!instructor) {
        throw new AppError('Unauthorized user', HttpStatusCodes.NOT_FOUND);
    }

    const isPasswordCorrect = await authService.comparePassword(password.currentPassword, instructor?.password);

    if (!isPasswordCorrect) {
        throw new AppError('Your current password is incorrect', HttpStatusCodes.BAD_REQUEST);
    }

    if (!password.newPassword) {
        throw new AppError('New password cannot be empty', HttpStatusCodes.BAD_REQUEST);
    }
...
export const getStudentsForInstructorsU = async (
    ...
    if (!instructorId) {
        throw new AppError('Please give a instructor id', HttpStatusCodes.BAD_REQUEST);
    }

    const students = await courseDbRepository.getStudentsByCourseForInstructor(instructorId);
