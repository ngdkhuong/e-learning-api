...
export const instructorRepoMongoDb = () => {
    ...
    const checkRejected = async (instructorId: string) => {
        const result = await Instructor.findOne({
            $and: [{ _id: new mongoose.Types.ObjectId(instructorId) }, { rejected: true }],
        });
        return result;
    };

    const rejectInstructorRequest = async (instructorId: string, reason: string) => {
        const options = {
            upsert: true,
            new: true,
        };
    ...
    const getAllInstructors = async () => {
        const instructors: SavedInstructorInterface[] | null = await Instructor.find({});
        return instructors;
    };
    ...
    const getTotalNumberOfInstructors = async () => {
        const total = await Instructor.find().countDocuments();
        return total;
    };

    const changePassword = async (id: string, password: string) => {
        await Instructor.updateOne({ _id: new mongoose.Types.ObjectId(id) }, { password });
    };
